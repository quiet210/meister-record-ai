...
- 학습모듈 선택 시 해당 모듈의 단원명 후보를 select로 제공하고, 단원은 선택하지 않아도 생성 가능
- 단원은 여러 개 선택할 수 있으며 선택 단원은 chip으로 표시하고 직접 입력 단원도 추가 가능
- 단원 후보 목록은 선택값이나 직접 입력값으로 덮어쓰지 않음
- 학습모듈 선택 시 학습모듈명, 단원명, 성취기준, 핵심키워드 기준 참고 성취기준을 최대 5개 미리보기로 표시
- 활동유형, 역량키워드, 보완점은 필수 선택이 아니며, 교사 관찰 메모 또는 선택 항목 중 하나 이상 있으면 생성 가능
- 성취기준 후보는 선택 단원이 있으면 해당 단원들을 우선 사용하고, 단원 미선택 시 `subject_name + learning_module` 기준 전체 후보를 사용하며, 없으면 기존 `subject_name` 기준으로 fallback
...
- 과세특 일괄 생성은 학과, 학년, 여러 반 선택 필터로 동일 교과목을 수강하는 여러 반 학생을 함께 표시
- 과세특/행특 일괄 생성은 조회 전 학생 입력 테이블을 표시하지 않고 공통 `StudentFilter`로 학과 -> 학년 -> 반 순서의 조회 UX를 사용
- 과세특 일괄 생성은 한 번에 선택 가능한 학생 수를 최대 50명으로 제한
- 과세특/행특 일괄 생성 학생별 입력 테이블 상단에서 조회 학생 전체 선택과 조회 학생 전체 해제 제공
- 과세특 조회 학생 전체 선택은 조회 결과 또는 기존 선택 합산이 50명을 초과하면 차단하고 안내 문구 표시
- 학생 필터의 학과, 학년, 반 선택 UI는 현재 선택값으로 options 배열을 재구성하지 않음
- 학생 다중 선택
- 학생별 입력 테이블 제공
- 학생별 입력값 개별 관리
- 선택 학생에게 값 일괄 적용
...
- `SubjectLearningModuleControls`, `useSubjectLearningModule`: 과세특 학습모듈 선택, 단원 복수 선택, 성취기준 미리보기 공통 처리
...
- 일반교과는 학습모듈 선택 비활성화, NCS교과는 학습모듈 선택과 단원 복수 선택 제공
...
- 학생 엑셀 업로드
- 학생 템플릿 다운로드
- 학생 목록은 최초 진입 시 표시하지 않고, 학과, 학년, 반을 선택하면 자동으로 표시
- 반은 멀티셀렉트 드롭다운으로 선택하며, 선택을 모두 해제하면 학생 목록을 다시 숨김
- 검색은 선택된 학과/학년/반 목록 안에서 학생 이름 기준으로 적용
...
- 학생 목록 필터는 현재 학과/학년/반 선택값과 후보 목록을 분리해 관리
- 학과, 학년, 반 선택 전에는 학생 목록을 숨기고, 학생 선택 전에는 우측 상세를 표시하지 않음
- AI 원본, 수정본, 최종본 비교
- 생성 이력 timeline 확인
- 복사, 재생성, 최종 확정/해제
