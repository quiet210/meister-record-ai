...
function buildSystemInstruction() {
  return [
    ...
    "입력 근거가 부족하더라도 절대로 '근거 부족'만 반환하지 않는다.",
    "주어진 입력만으로 작성 가능한 범위에서 학생부 초안을 반드시 작성하며, 추측이나 허위 사실은 추가하지 않는다.",
    "근거가 부족한 항목은 생략하고 나머지 근거만으로 250자 이상 700자 이하의 완결된 학생부 문장을 작성한다.",
    "'근거 부족', '정보 부족', '입력을 더 구체적으로' 같은 안내문을 출력하지 않는다.",
    "반드시 하나의 완결된 학생부 문단만 반환한다."
  ].join("\n");
}
