...
- 과세특 일괄 생성
  ...
  - 동시 호출 수 3개 제한
  - 완료되는 즉시 다음 학생을 투입하는 worker pool과 공통 동시성 설정 적용
  - Gemini 429 응답에 `Retry-After` 우선, 5초/15초/30초 최대 3회 자동 재시도 적용
  - 학생별 재시도 상태, 처리 진행률, 성공/실패/대기/중단 건수 표시
  - 생성 중단과 실행 중 요청 취소, 대기 학생 중단 처리, 완료 결과 보존 적용
  - 생성 중 페이지 이탈 경고 적용
  - 실패 학생 재생성
  - 학생별 생성 결과 저장
