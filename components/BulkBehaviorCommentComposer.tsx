...
const emptyStudentBehaviorInput = makeInitialStudentInput();
...
type BulkBehaviorStudentRowProps = {
  student: Student;
  input: StudentBehaviorInput;
  departmentName: string;
  selected: boolean;
  isGenerating: boolean;
  schoolLifeAreaOptions: readonly string[];
  lifeAttitudeOptions: readonly string[];
  relationshipOptions: readonly string[];
  responsibilityOptions: readonly string[];
  behaviorImprovementOptions: readonly string[];
  onToggleStudent: (studentId: string) => void;
  onPatchInput: (studentId: string, patch: Partial<StudentBehaviorInput>, resetResult?: boolean) => void;
  onCopyPrevious: (studentId: string) => void;
  onGenerate: (student: Student) => void;
};

const BulkBehaviorStudentRow = memo(function BulkBehaviorStudentRow({
  student,
  input,
  departmentName,
  selected,
    ...
    <tr className={`align-top ${selected ? "bg-blue-50/30" : ""}`}>
      ...
      <td className="px-3 py-3">
        <ChipSelector
          ...
          onChange={(values) => onPatchInput(student.id, { schoolLifeAreas: values })}
      ...
      <td className="px-3 py-3">
        <ChipSelector
          ...
          onChange={(values) => onPatchInput(student.id, { lifeAttitudeKeywords: values })}
          disabled={isRowGenerating}
          compact
          showLabel={false}
        />
      ...
      <td className="px-3 py-3">
        <ChipSelector
          ...
          onChange={(values) => onPatchInput(student.id, { relationshipKeywords: values })}
          disabled={isRowGenerating}
          compact
          showLabel={false}
        />
      ...
      <td className="px-3 py-3">
        <ChipSelector
          ...
          onChange={(values) => onPatchInput(student.id, { responsibilityKeywords: values })}
          disabled={isRowGenerating}
          compact
          showLabel={false}
        />
      ...
      <td className="px-3 py-3">
        <ChipSelector
          ...
          onChange={(values) => onPatchInput(student.id, { behaviorImprovements: values })}
          disabled={isRowGenerating}
          compact
          showLabel={false}
        />
      ...
      <td className="px-3 py-3">
        <textarea
          ...
          onChange={(event) => onPatchInput(student.id, { homeroomMemo: event.target.value })}
          disabled={isRowGenerating}
        />
      </td>
      <td className="px-3 py-3">
...
type BulkBehaviorResultCardProps = {
  student: Student;
  input: StudentBehaviorInput;
  departmentName: string;
  isGenerating: boolean;
...
const BulkBehaviorResultCard = memo(function BulkBehaviorResultCard({
  student,
  input,
  departmentName,
  isGenerating,
...
export function BulkBehaviorCommentComposer() {
  ...
  const selectedInputStats = useMemo(() => {
    ...
    selectedStudents.forEach((student) => {
      const input = studentInputs[student.id] || emptyStudentBehaviorInput;
  ...
  const generationProgress = useMemo(() => {
    const targetInputs = generationStudentIds.map((studentId) => studentInputs[studentId] || emptyStudentBehaviorInput);
  ...
  const behaviorResultExportRows = useMemo<BehaviorCommentResultExportRow[]>(
    ...
    [departmentLabelMap, selectedStudents, studentInputs]
  );
  const completedExportResultCount = useMemo(
    () => behaviorResultExportRows.filter((row) => row.generationStatus === "completed" && row.draft.trim().length > 0).length,
    [behaviorResultExportRows]
  );
  ...
  const patchStudentInput = useCallback((studentId: string, patch: Partial<StudentBehaviorInput>, resetResult = true) => {
    setStudentInputs((current) => {
  ...
  async function generateForStudents(
    ...
    await runBulkGenerationQueue(runnableStudents, async (student) => {
      ...
      try {
        const result = await generateWithRateLimitRetry("/api/generate/behavior-comment", payload, {
          ...
          <div className="flex flex-col gap-2 lg:items-end">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {failedStudents.length > 0 ? (
                <button className="secondary-button" type="button" onClick={regenerateFailedStudents} disabled={!hasStudentLookupCriteria || isGenerating}>
